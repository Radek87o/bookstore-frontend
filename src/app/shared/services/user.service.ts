...
export class UserService {
  ...
  updateUser(user: SignupDto, isActive: string, isNonLocked: string, role: string, currentUsername: string) : Observable<User> {
    let activeParam = isActive===null ? 'true' : isActive;
    let nonLockedParam = isNonLocked===null ? 'true' : isNonLocked;
    let roleParam = role===null ? 'user' : role;
    let queryParams = new HttpParams()
                  ...
                  .set('isNonLocked', nonLockedParam)
                  .set('role', roleParam);
    return this.http.post<User>(`${this.baseUrl}/update`, user, {params: queryParams})
  }
